...
public final class ManyThreadsCalculator {
        ...
        for (int i = 0; i < threadsCount; i++) {
            Thread thread = new Thread(() -> {
                ...
                while (totalCount.get() < pointsCount) {
                    x = ThreadLocalRandom.current().nextDouble(0, SQUARE_SIDE_LENGTH);
                    y = ThreadLocalRandom.current().nextDouble(0, SQUARE_SIDE_LENGTH);
                    if (isInCircle(x, y)) {
